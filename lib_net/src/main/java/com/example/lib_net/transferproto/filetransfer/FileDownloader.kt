...
class FileDownloader(
    ...
    private inner class SingleFileDownloader(val file: FileExploreFile) :
        ...
        fun onCanceled(reason: String, reportRemote: Boolean) {
            if (isSingleFileDownloaderExecuted.get() && !isSingleFileFinished.get() && isSingleFileDownloaderCanceled.compareAndSet(
                    false,
                    true
                )
            ) {
        ...
        private suspend fun SingleFileFragmentDownloader.connectToServerSuspend(): Unit =
        ...
        private inner class SingleFileFragmentDownloader(
                            ...
                            if (!isFragmentDownloaderClosed.get() && !isFragmentDownloaderFinished.get()) {
                                try {
                                    ...
                                    if (downloadedSize.addAndGet(data.size.toLong()) >= size) {
                                        log.d(
                                            TAG,
                                            "Frame: $start download finished(${end - start} bytes)."
                                        )
                                        ...
                                        if (t != null) {
                                            t.requestSimplify(
                                                ...
                                                retryTimeout = 2500,
                                                callback = object :
                                                    IClientManager.RequestCallback<Unit> {
                                                    ...
                                                    override fun onFail(errorMsg: String) {
                                                        ...
                                                        log.e(
                                                            TAG,
                                                            "Send fragment finish fail: $errorMsg"
                                                        )
                                                        // closeConnectionIfActive()
                                                    }
            ...
            fun isActive(): Boolean =
                task.get()?.getCurrentState() is NettyTaskState.ConnectionActive
